...
def editors_list():
    ...
    rows = [dict(r) for r in conn.execute(
        "SELECT id, name, email, role, subjects, active, password_hash, last_login FROM editors ORDER BY active DESC, subjects, name"
    ).fetchall()]
    for r in rows:
        if r["subjects"]:
            try:
...
def editor_set_password(eid):
    ...
    if not password or len(password) < 4:
        ...
        return redirect(url_for("editors_list"))
    h = generate_password_hash(password)
    conn.execute("UPDATE editors SET password_hash=? WHERE id=?", (h, eid))
    conn.commit()
