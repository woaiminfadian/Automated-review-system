...
- **主编工作台** — 左侧以审稿流程为主线导航：系统总览 / 收稿管理 / 一审管理 / 二审管理 / 三审管理 / 人员管理，轮次页面支持分页、搜索/学科筛选、状态快捷筛选（待派稿、待审回、部分审回、待主编处理、待通知作者、已结束）
- **编辑审稿** — 在线评分（选题/观点/论证/创新/规范性/引用实证/结构文字，6维100分制，总分自动计算）、审稿意见、上传评分表和批注版稿件，支持盲审模式，评分表模板一键下载
...
│   │   ├── create_db.py        # 数据库初始化
│   │   ├── migrations/             # 数据库迁移脚本
│   │   │   ├── migrate_v5_workflow.py       # v5: 审稿流程主线（review_rounds、author_notifications）
│   │   │   └── migrate_v7_workflow_reconcile.py  # v7: 工作流状态对齐、论文信息字段
...
```bash
cd 自动化审稿系统/后端审稿系统
python3 migrations/migrate_v5_workflow.py           # v5: 新增 review_rounds、author_notifications 表，审稿流程主线优化
python3 migrations/migrate_v7_workflow_reconcile.py # v7: 工作流状态对齐、论文信息字段
```

迁移前自动备份 `journal.db → journal.db.bak.v{N}`。
