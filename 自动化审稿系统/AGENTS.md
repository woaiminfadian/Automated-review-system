...
dashboard.html               管理员总览（含局域网访问地址）
intake.html                  收稿管理（待登记邮件/待匿名/已匿名待一审）
review_round_manage.html     轮次管理（一审/二审/三审共用）
submission_detail.html       稿件详情（含轮次卡片、主编决定、作者回复）
...
people.html                  人员管理（作者+编辑 tab）
login.html / setup.html      登录与首次设置
```

> 旧页面 `email_inbox.html`、`submissions.html`、`assignments.html`、`authors.html`、`editors.html` 已删除，对应路由重定向到新入口。
...
- `submission_files` 表已实现，记录每篇稿件的全部附件（含文件类型、版本标签、轮次）。
- `submissions.file_path` 保留兼容，但新代码优先使用 `submission_files`。
- 编辑端可上传审稿评分表和批注版，稿件附件通过 `submission_files` 管理。
- 附件下载有基于角色的权限控制：管理员全权限，编辑仅限分配给自己的稿件。
...
*.pyc
.DS_Store
自动化审稿系统/.automation/   （运行时数据、邮件原文、草稿）
自动化审稿系统/审稿评分表.docx
```

`root_dir` 配置为 `".."`，即所有相对路径（state_dir、模板文件等）均解析到 `自动化审稿系统/` 目录内，不会溢出到学报根目录。
...
2. 完善编辑端下载原稿/附件、上传审稿文件的闭环。
3. 邮件收稿增加后台轮询服务，实现接近实时处理。
4. 前端 UI 改为紧凑工作台：减少大卡片、强化流程按钮联动。
5. 取消固定默认密码或强制首次登录修改。
6. 多人局域网使用时，关闭 debug，设置稳定 SECRET_KEY。
...
10. **所有系统相关文件（代码、配置、模板、数据库、运行时数据、启动脚本）必须归入 `自动化审稿系统/` 文件夹，不得溢出到学报根目录。** 根目录仅保留学报内容（如 `0. 学报面试/`、`1. 未处理来稿/` 等），不放置任何系统文件。
